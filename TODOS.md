...
7. [ ] **IC daily snapshot semantics** (2026-07-25) — `docs/plan/paper-ic-daily-snapshot/tasks.md`, starting at **SNAP-1** (Owner: Claude — financial-logic gate). **2026-08-07:** SNAP-2 (wire `record_leg_snapshot()` into `paper_ic_snapshot.py`) closed without implementation — `paper_nav_snapshots` already has strategy-level daily P&L for all five IC variants, no code change needed to hit the story's four end goals. SNAP-4 no longer blocked by SNAP-2 (still blocked by SNAP-1). See `stories.md` SNAP-2 finding, `DB_REGISTRY.md`.
