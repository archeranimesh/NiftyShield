...
- Followed `docs/bugs/prompt.md` protocol; picked up BUG-028 Phase 4, flagged in `bugs.md` the
  ...
  INFO note logged in `bugs.md`. **Commit deferred to live host**: this device sandbox has no
  `pre-commit` installed and no network to install it (`.venv/bin/python` here is a broken
  symlink to the live host's `/opt/anaconda3/bin/python`) — code, tests, and review are complete
  and staged (`src/strategy/auto_close.py`, `tests/unit/strategy/test_auto_close.py`); Animesh
  needs to run `git commit` on the live host and backfill the SHA into `docs/bugs/task.md`
  B028.17, `docs/bugs/bugs.md`'s Phase 4 line, and this entry. See `docs/bugs/task.md`
  B028.14–B028.17.
