...
6. [ ] **Paper exit codification** (2026-07-27, updated 2026-08-02) — `docs/plan/paper-exit-codification/tasks.md`. EC-1 retired 2026-08-02 (superseded by EC-5, confirmed no other evaluator shares its gap). EC-2 shipped 2026-08-02. EC-4 (CSP portion — `evaluate_time_stop_csp` now gates on DTE-remaining, reusing `ReEntryMixin`'s DTE≥14 floor) implemented 2026-08-02; not run in-sandbox (disk quota, ENOSPC), needs live-host `pytest` confirmation. Next: **EC-5** (CC-only flat DTE≤5 close, operator decision 2026-08-01, not yet implemented) then **EC-3** (docs close). **Cross-epic note:** item 1's CC1/CC2/CC3 sub-thread depended on EC-4 landing first for TIME_STOP semantics — that dependency is now satisfied (CSP portion landed); confirm before resuming CC2/CC3.
