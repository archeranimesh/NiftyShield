...
- 2026-08-25: MD-7.2 - blanket-escaped `_gate_alert` messages in `paper_ic_entry.py` and `paper_ic_entry_v2.py`. Satisfied `test_escaping_guard.py` by placing the escaping boundary natively inside `_gate_alert`, removing both baseline entries. Added unit tests for each script. SHA `6e66f7f`.
