...
1. [ ] **3-Track Consolidation & Automation** (2026-07-21, revised through 2026-07-30 — S1r/S2r/S3/S3r/S4/S5/S6 all shipped; **only S0 (docs/decision-log close-out) remains open** on the S-numbered thread as of 2026-08-01) — `docs/plan/3track-consolidation/`. Original S1/S2 were superseded 2026-07-29 by **S1r** (overlay legs re-homed to track-independent `paper_nifty_overlay` namespace, SHA 8c41cca — this same commit also fixed the S1b `overlay_cc` state bug, closed) and **S2r** (both track-ownership overlay blocks removed, SHA abdb7ef). **S3** (independent daily base-leg comparison snapshot, SHA 07570d3) and **S3r** (query-time overlay coverage/P&L per track, SHA 450cd9c) shipped 2026-07-29. **S4** (`NiftyTrackComparisonV1.auto_execute=True`, SHA ef540c7) and **S5** (automated base-leg rolling for Futures/DITM — `base_futures` DTE≤1, `base_ditm_call` DTE<20, warn-only liquidity gate, SHA 177660e) shipped 2026-07-29/30. **S6** (full unattended automation — one-time bootstrap entry + Telegram notification on every trade event, SHA 297922e) shipped 2026-07-30, closing the last human approval gate in the pipeline. **No open PP-booking-gap or CC-state-bug items remain** — both classes of defect this epic was tracking were fixed under S1r (see `docs/archive/TODOS_ARCHIVE.md` 2026-07-29 entry). Remaining work: **S0** — rewrite `docs/instructions/3track.md`/`docs/archive/strategies/nifty_track_comparison_v1.md` to retire RQ2 explicitly and describe the single-copy overlay + full-automation model (docs-only, no code-reviewer gate — see `docs/plan/3track-consolidation/tasks.md`). See `DECISIONS.md` 2026-07-28 through 2026-07-30 entries for full reasoning. **CC sub-thread
added same day (independent of S1–S6/S0's sequencing, but tracked in this same folder since
CC is the overlay this epic restricts/automates):** **CC1 shipped 2026-08-01** (CC-specific
delta ladder for `find_strike_by_delta.py`, previously misapplied CSP's ladder to CC — new
`CC_DELTA_CANDIDATES` is explicitly provisional pending CC2, see DECISIONS.md 2026-08-01 CC1
entry). Next open item on this sub-thread: CC2 (decision gate —
...
6. [x] **Paper exit codification** (2026-07-27, closed 2026-08-02) — `docs/plan/paper-exit-codification/tasks.md`. All tasks resolved: EC-1 retired (superseded by EC-5), EC-2 shipped, EC-4 (CSP portion) implemented, EC-5 (CC-only flat DTE≤5 close) implemented, EC-3 (docs close) done — this line is that close-out. **Open verification debt carried forward:** EC-4/EC-5 changes were never run through `pytest` in-sandbox (disk quota exhausted both sessions) — needs a live-host `pytest tests/unit/` pass before fully trusting those two changes. **Cross-epic note:** item 1's CC1/CC2/CC3 sub-thread's EC-4 dependency is satisfied (CSP portion landed); confirm before resuming CC2/CC3.
