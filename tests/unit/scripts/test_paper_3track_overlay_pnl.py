"""Unit tests for S8: daily P&L comparison table for CC/PP/Collar overlays.
...
- _compute_overlay_pnl_snapshots: persists a snapshot per overlay_type present
  today, reading only the real-leg-role paper_leg_snapshots rows S7 produces.
...
- Collar's call + put legs merge into a single "collar" row, matching
  _normalize_overlay_pnls' display convention.
...
def test_overlay_pnl_snapshot_persists_all_three_types_daily(tmp_path: Path) -> None:
    ...
    ``overlay_cc`` is deliberately omitted here — per ``_normalize_overlay_pnls``
    ...
    results = _compute_overlay_pnl_snapshots(store, _TRACK, snap_date)
...
def test_overlay_pnl_1d_uses_yesterday_mark_denominator(tmp_path: Path) -> None:
    ...
    results = _compute_overlay_pnl_snapshots(store, _TRACK, day2)
...
def test_overlay_pnl_inception_uses_entry_basis_denominator(tmp_path: Path) -> None:
    ...
    store.record_leg_snapshot(_leg_snap("overlay_pp", Decimal("1500"), snap_date, ltp=Decimal("80")))
...
def test_queryable_by_strategy_and_overlay_type(tmp_path: Path) -> None:
    ...
    for snap in _compute_overlay_pnl_snapshots(store, _TRACK, snap_date):
...
def test_overlay_with_no_legs_produces_no_rows(tmp_path: Path) -> None:
    ...
    store = _store(tmp_path)
    results = _compute_overlay_pnl_snapshots(store, _TRACK, date(2026, 5, 2))
    assert results == []
