"""Tests for scripts/upstox_chain_intraday.py — 5-min intraday chain snapshot cron.
...
def test_happy_path_three_expiries() -> None:
    """Trading day with 3 expiries: write_intraday_snapshot called 3×, returns 0."""
    mock_chain = _make_mock_chain(n_strikes=5)
    mock_lookup = _make_mock_lookup(THREE_EXPIRIES)
    written_path = Path("/tmp/some/upstox_1430.parquet")
    ...
    with (
        ...
        mock_writer_cls.return_value.write_intraday_snapshot.return_value = written_path

        result = main()

    assert result == 0
    assert mock_writer_cls.return_value.write_intraday_snapshot.call_count == 3
...
def test_single_expiry_failure_continues() -> None:
    ...
    assert mock_writer_cls.return_value.write_intraday_snapshot.call_count == 2
...
def test_snapshot_ts_is_utc_aware() -> None:
    """snapshot_ts passed to write_intraday_snapshot is timezone-aware UTC."""
    ...
    call_args = mock_writer_cls.return_value.write_intraday_snapshot.call_args
...
def test_base_dir_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """CHAIN_INTRADAY_DIR env var is passed to ChainWriter constructor."""
    custom_dir = "/tmp/test_chain_snapshots_5min"
    monkeypatch.setenv("CHAIN_INTRADAY_DIR", custom_dir)
...
def test_label_passed_per_expiry() -> None:
    """BUG-006 regression: each write_intraday_snapshot call gets its own label kwarg.

    Same snapshot_ts (same HHMM) across all 3 expiries — the label is the only thing
    ...
    assert result == 0
    calls = mock_writer_cls.return_value.write_intraday_snapshot.call_args_list
    assert len(calls) == 3
