...
def test_load_options_ohlcv_date_filter(bhavcopy_zip, tmp_path):
    ...
    df_empty = load_options_ohlcv(
        underlying="NIFTY", start=outside_date, end=outside_date, data_dir=tmp_path
    )
    assert len(df_empty) == 0


def test_load_options_ohlcv_empty_on_no_data(tmp_path):
    df = load_options_ohlcv(
        underlying="NIFTY", start=date(2020, 1, 1), end=date(2020, 1, 31), data_dir=tmp_path
    )
    assert len(df) == 0
...
def test_bhavcopy_bootstrap_404_handling(mock_sleep, mock_download, tmp_path):
    ...
    args = ["--start", "2024-04-24", "--end", "2024-04-24", "--dest", str(tmp_path)]
...
def test_bhavcopy_integration_end_to_end(mock_sleep, mock_download, bhavcopy_zip, tmp_path):
    ...
    # 1. Options Parquet
    options_df = load_options_ohlcv(
        "NIFTY", date(2024, 4, 25), date(2024, 4, 25), tmp_path / "options_ohlcv"
    )
    ...
    # 2. Futures Parquet
    futures_df = load_options_ohlcv(
        "NIFTY", date(2024, 4, 25), date(2024, 4, 25), tmp_path / "futures_ohlcv"
    )
