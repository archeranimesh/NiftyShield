...
> skill**. Deferred, not downgraded: invoke it when a step below points there.
...
A second `Read` of a path already read this session (with no intervening `Edit`/`Write`) is
flagged by `repeat_read.sh` — edit the copy already in context, or use `get_code_snippet` /
...
filter before output reaches Claude context. Raw result sets are appended to the context
...
**Shell mechanics.** The session shell is **zsh** — no bash-only builtins (`mapfile`,
`readarray`), and never rely on `$var` word-splitting for newline lists (use `while read` /
`xargs -0` / a one-shot Python filter). **Never put `cd` in a compound Bash command** — the
working directory persists across Bash tool calls, so `cd sub && grep …` silently breaks the
next relative-path call. Use absolute paths or `git -C <dir>` / `--` path args.
...
picks the target off `TODOS.md` `## Feature Backlog` (feature) or `docs/bugs/` (bug), loads
the story/bug prompt + first unchecked task + `CONTEXT.md`, then hands to Step 2b below.
...
This three-condition manual check is the **sole source of truth** for the checkpoint —
`options-strategist` is advisory-only, and a real council call per `docs/council/README.md`
always supersedes it when the three conditions hold. This checkpoint exists only in the
planning phase; never invoke the council mid-implementation.

**Reading a council file that already exists is a separate mandatory gate — invoke
`protocol-reference` §1 before acting on any `docs/council/` output.**
...
**Claude implements** → proceed to Step 4. AutoTrigger agents fire during and after
implementation; Claude commits via the `commit` skill.

**Antigravity implements** → **settle this routing call firmly before invoking
`handoff-antigravity`** — its `SKILL.md` body (~1.5K tokens) is only useful once the decision
is "Antigravity"; loading it and then reversing to "Claude implements" is pure waste. Invoke
the skill, produce the structured handoff prompt, and **stop**. Write no code. Antigravity
runs its own protocol (TDD loop, persona review, commit) and returns a Phase Completion
Output block. Claude then verifies the SHA matches `git log --oneline -1` and the test count
meets DoD; if either fails, open a fix session with the failure details. Handoff-prompt
requirements and the full verification checklist: `protocol-reference` §3.
...
Spawning the correct sub-agent is not optional for the conditions below. Inline review is not
a substitute — each agent runs in an isolated context with the right model.
...
`@code-reviewer` subagent is mandatory — Antigravity's persona approximation is insufficient
because it does not load `REVIEW.md` hygiene rules unless explicitly provided.
...
**5a — Update docs** (targeted `Edit` calls only, never `Write`): `CONTEXT.md` "What Exists"
if new files were added · `DECISIONS.md` for any new architecture decision · `TODOS.md` to
mark completed items and add a session-log entry · `docs/plan/README.md` status column for the
story/epic just touched (a stale epic is almost always this file being skipped — FR-7 row 15,
`docs/plan/full-repo-review/findings/FR-7_synthesis.md`) · the relevant
`src/<module>/CLAUDE.md` if module invariants changed. A completed `tasks.md` checkbox carries
a `| Owner: … | Model: … | Review: … | SHA: …` tail (`Owner`/`Model`/`Review` set at authoring
time, `SHA` on the closing commit); `TODOS.md` backlog items stay pointer-only. Full rules:
...
under `src/`, `scripts/`, or `tests/` runs the `code-reviewer` agent against `git diff HEAD`
first — resolve every `CRITICAL` / `ERROR`; a `WARNING` may be deferred with a documented
reason. A docs/config-only diff (no such `.py` files) skips `code-reviewer` and commits
immediately after 5a. **Never bundle changes from separate phases into one commit.** Typical
phase boundaries, each getting its own commit: Model → Store → Tracker/orchestration →
...
optional when it applies. Invoke `protocol-reference`
(`.claude/skills/protocol-reference/SKILL.md`) for:
...
**§1 is a gate, not a lookup** — load it whenever a `docs/council/` file is shared or
