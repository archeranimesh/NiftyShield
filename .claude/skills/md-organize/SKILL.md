...
`DECISIONS.md` is Step 4's responsibility, not this sweep's. `suggestions.md` is exempt — it is a wide fixed-column table owned by `session-close` Step 4b; commit it with `SKIP=md-line-length` and do
not reflow its rows.
...
1. **`AGENTS.md`** — Antigravity's autoload protocol, a full standalone mirror. Re-apply each `CLAUDE.md` edit to the matching passage. Preserve the intentional deltas: the "Antigravity autoload /
   ...
   differ: `diff <(sed 's/[[:space:]]*$//' CLAUDE.md) <(sed 's/[[:space:]]*$//' AGENTS.md)`. Before writing any file path into `AGENTS.md` or `.agents/**`, grep an existing mirror for the convention
   first — this repo's mirrors reference `.claude/` paths, never their own directory; assuming otherwise silently produces dangling pointers on the mirrored surface.
