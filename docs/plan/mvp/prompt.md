Read `CONTEXT.md` and state `CONTEXT.md ✓` before doing anything else. Then read `docs/plan/mvp/mvp_tasks.md` and find the first unchecked box
...
**Story spec:** Read the matching story in `docs/plan/mvp/mvp_stories.md` (same task ID) for the full implementation spec,
...
**Verify and record:** Copy the SHA from `git log --oneline -1`. Open `docs/plan/mvp/mvp_tasks.md`, change `- [ ]` to `- [x]` on the completed line,
and append `| SHA: <sha>`. Then add one line to `TODOS.md` under the session log:
`- [YYYY-MM-DD] MVP <task-id> — <one-line description> — <SHA>`
