Read `CONTEXT.md` and state `CONTEXT.md ✓` before doing anything else. Then read `docs/plan/covered-call-overlay/tasks.md` and find the first unchecked box — the first `- [ ]` line. That is your **only task** for this session. Do not look at any other unchecked item. Do not attempt to batch or combine tasks. One task. Complete it fully. Move on to nothing else.

**Story spec:** Read the matching story in `docs/plan/covered-call-overlay/stories.md` (same task ID) for the full implementation spec, "Before any code" graph queries, test list, and commit message. Follow it exactly.

**Strategy context:** Read `docs/strategies/covered_call_overlay_v1.md` if your task involves entry rules, exit triggers, IVR gate, or quantity constraints — the strategy doc is the authoritative source for all parameters. Do not hardcode values from memory.
...
**Verify and record:** Copy the SHA from `git log --oneline -1`. Open `docs/plan/covered-call-overlay/tasks.md`, change `- [ ]` to `- [x]` on the completed line, and append `| SHA: <sha>`. Then add one line to `TODOS.md` under the session log:
`| YYYY-MM-DD | CC task-id — one-line description — SHA |`
