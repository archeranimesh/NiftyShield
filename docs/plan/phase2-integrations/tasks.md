...
- [ ] **PV-1** — P&L Visualization (Cowork artifact). ~6 weeks of data available now — buildable if prioritised ahead of the Phase 1.12 gate,
since it only reads existing data. Four panels: MF, Dhan ETFs, Nuvama Bonds, Nuvama Options. Panel 5 (Zerodha) blocked on Kite Connect (see ZK-1).
...
- [ ] **OE-1** — Order Execution Layer (`src/execution/`). Blocked: static IP not provisioned (`_raise_order_blocked()` in `src/client/upstox_live.py`, per DECISIONS.md).
Design already done against `BrokerClient` protocol.
- [ ] **PT-1** — `paper_snapshot.py` → Telegram. Wire `build_notifier`; non-fatal.
Defer until the file is touched for another reason (confirmed: no notifier wiring exists in `paper_snapshot.py` yet, as of 2026-07-27).
