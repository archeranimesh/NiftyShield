Read `CONTEXT.md` and state `CONTEXT.md ✓` before doing anything else. Then read `docs/plan/options_income/options_income_tasks.md` and find the first unchecked box — the first `- [ ]` line. That is your **only task** for this session. Do not look at any other unchecked item. One task. Complete it fully. Stop.
...
**Story spec:** Read the matching story in `docs/plan/options_income/options_income_stories.md` (same task ID) for the full implementation spec, "Before any code" graph queries, test list, and commit message. Follow it exactly.
...
**greeks-analyst gate (mandatory for S2, S3, S6):** After implementation, before code-reviewer, spawn the `greeks-analyst` agent against the changed files. Any CRITICAL finding must be resolved before proceeding.
