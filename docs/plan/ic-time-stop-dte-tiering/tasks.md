...
- [ ] **DT-1** `[Claude]` — Update `ic_expiry_config.py` CONFIGS: `time_stop_dte=7`/`dte_warn=14` for monthly/leaps/yearly (weekly unchanged) + test run
- [ ] **DT-2** `[Claude]` — Docs: `DECISIONS.md` entry + `IC-M1.md` correction note (docs only, no code-reviewer gate)
- [ ] **DT-3a** `[Claude]` — Audit: confirm the actual write path for IC `paper_exit_events` rows (`create_exit_event` has no confirmed IC-side caller yet — see stories.md). Ambiguous, exploratory, must resolve before DT-3b can be handed off.
