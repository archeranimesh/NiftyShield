...
- [x] **S3r** — Query-time overlay coverage/P&L per track (Spot/Futures/Proxy), new story, not in original scope. Effective-Nifty-exposure calc (ETF ≈1x, Futures via margin/SPAN, DITM via live delta) — no qty/lot resizing. Depends on: S1r (overlay must be re-homed before a track-agnostic query makes sense). **Deviation:** Futures uses `qty * 1.0` delta, not `paper_margin_snapshots` (that table is IC-only, never written by the 3-track path) — confirmed with operator, see `DECISIONS.md` 2026-07-29 S3r entry. | SHA: (pending)
