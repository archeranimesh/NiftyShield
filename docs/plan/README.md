...
| `root-doc-organization/` | Token-efficiency cleanup of the ~22 root `.md` files + maintenance automation: slim `CONTEXT.md` (done), re-sync `AGENTS.md` to `CLAUDE.md` (done), shrink `DECISIONS.md`, line-length pre-commit guard, `md-organize` on-demand skill, doc-staleness automation. Docs+tooling only, no `src/` behavior change. RDO-1..10. | RDO-4 / RDO-9 (DECISIONS semantic split) | 🔄 In progress — RDO-1 (CONTEXT.md 81K→10K) + RDO-2 (AGENTS.md re-sync) shipped 2026-08-27; RDO-3 partial (7bbfaff — 5 historical sections → `docs/archive/DECISIONS_pre-2026-07.md`), date-cutoff found unworkable → **RDO-9** semantic split; **RDO-10** reconciles RDO-7 with the SessionStart freshness hook + commit gate a parallel epic shipped 2026-08-27; RDO-4/5/6/7/8 open |
