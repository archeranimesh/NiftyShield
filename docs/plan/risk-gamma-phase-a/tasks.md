...
> `| Owner: <Claude|Antigravity|Animesh> | Model: <model-id|n/a> | SHA: <sha>` when done.
> Add one line to `TODOS.md` session log. See `docs/plan/README.md` §Conventions.
> Full story spec for each task: `docs/plan/risk-gamma-phase-a/stories.md`.

---

## Track A — Delta Gate Wiring

- [x] **A** — Wire `src/risk/` delta gate into `record_paper_trade.py` | Owner: (pre-convention) | Model: n/a | SHA: b9c00146
...
- [x] **B1** — `src/gamma/` package: models (`GammaChainSnapshot`, `GammaWatchlistEntry`) + `GammaStore` | Owner: (pre-convention) | Model: n/a | SHA: d8c2e69

### Phase B2 — `scripts/gamma_daily_watch.py` (5 sub-tasks, one session each)

- [x] **B2.1** — Script scaffold: CLI flags + expiry resolution | Owner: (pre-convention) | Model: n/a | SHA: b68bb3d
