...
**Surface & Model: Antigravity (handoff via `handoff-antigravity`); Claude Code / Sonnet for the review half.** Mechanical, grep-verifiable link/table fixes across 4 docs — within `ai_collaboration_plan.md`'s 3-5 file ceiling for Antigravity. Docs-only: skip the code-reviewer gate per Step 5c, but Claude should still `grep` for the dead paths after the handoff returns to confirm no other doc still references them.
