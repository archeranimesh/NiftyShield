...
4. **Boundaries** — Do not touch `data/portfolio/portfolio.sqlite` itself except through the read-only `.backup` API — no writes, no schema changes. Do not touch any file outside `scripts/portfolio/`
   and its test file.
