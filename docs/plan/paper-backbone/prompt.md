...
**Verify and record:** Copy the SHA from `git log --oneline -1`. Open `docs/plan/paper-backbone/paper_backbone_tasks.md`, change `- [ ]` to `- [x]` on the completed line, and append `| SHA: <sha>`. Then add one line to `TODOS.md` under the session log:
`| <YYYY-MM-DD> | paper-backbone <task-id> — <one-line description> — <SHA> |`
