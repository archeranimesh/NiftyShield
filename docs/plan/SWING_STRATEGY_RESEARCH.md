...
| 2026-05-02 | Strategy 3 | Council review | 3-model council (GPT-4.1 + DeepSeek-R1 joint top-ranked at avg rank 1.5; Grok-4-fast at 3.0; chairman: Claude Sonnet 4.6 reconstructed after Stage 3 synthesis failure). Four binding decisions: (1) Gap Fade IVP threshold confirmed at **75th percentile** (`vix_ivp_threshold = 0.75`) — not raised to 80th; (2) Asymmetric thresholds with ORB (90th) are **mandatory** — unified threshold rejected on structural grounds (different failure modes: continuous gradient vs. binary tail); (3) Primary contingency for insufficient windows: **expand gap range 0.3–1.0% → 0.25–1.2%** before raising threshold; secondary: raise threshold ≤ 0.85 in 0.05 steps if primary fails; (4) **Mandatory ablation**: report Sharpe at [0.70, 0.75, 0.80, 0.85] in Phase 1 walk-forward; retain 0.75 if delta vs. 0.80 ≥ 0.10 Sharpe, raise to 0.80 if delta < 0.10. No code or spec text changes — `vix_ivp_threshold = 0.75` was already in §2c. Full decision: `docs/council/2026-05-02_gap-fade-vix-filter-threshold.md`. |
