...
- [x] **B027.1** — Added `from dotenv import load_dotenv` + a module-level `load_dotenv()` call to `scripts/healthcheck.py`, placed before the `src.*` imports (mirrors `scripts/eod_summary.py`'s exact placement — comment explaining why, `# noqa: E402` on the now-late `src.*` imports, same convention). | No SHA yet — pending commit
