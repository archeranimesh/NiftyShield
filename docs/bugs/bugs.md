...
| Status | ✅ Fixed (moot — root cause pre-existed the bug report) |
...
**Resolution (2026-07-03, investigation-only, no code change):** root cause was already fixed a month before this bug was filed. `fe69612` (2026-05-30, "replace direct environment access with Settings singleton") added the exact guard the leading hypothesis called for — `_DynamicSettings._get_settings` (`src/config.py:205`) forces `kwargs["_env_file"] = None` whenever `"pytest" in sys.modules` (or `UPSTOX_ENV == "test"`), so a real host-exported `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` or stray `.env` can't leak into `settings` under test. Repo-wide `grep -rn "os.environ\[" tests/` found zero raw mutations of these two vars — every test in `test_notifications.py` already uses `monkeypatch.setenv`/`delenv` exclusively, which pytest auto-reverts per-test regardless of suite ordering. `_DynamicSettings`'s `hash(frozenset(os.environ.items()))` invalidation was traced and confirmed to recompute correctly on every env change; `build_notifier()` (`src/notifications/telegram.py`) has no independent caching that could bypass it. The originally-reported failure does not reproduce against current `HEAD` — closing as fixed/moot, citing `fe69612` as the (pre-existing) fix commit. If this resurfaces on the live host, it points at something more specific than a missing isolation guard (e.g. a genuinely different test-run environment) and should be re-opened as a new bug with the actual live-host repro output attached.
