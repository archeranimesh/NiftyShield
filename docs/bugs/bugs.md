...
**Fix:** ORD-1 determines whether the two can genuinely coexist and whether a reliable marker distinguishes "collar call tagged `overlay_cc`" from "standalone `overlay_cc`", then decides the correct
`_overlay_type_groups` behaviour; ORD-2 implements it so a standalone CC gets its own group/row and the collar total excludes it, without regressing BUG-030's own test.
