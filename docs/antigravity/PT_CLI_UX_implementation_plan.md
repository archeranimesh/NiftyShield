...
> **Post-phase step:** run `mcp__codebase-memory-mcp__index_repository` to register
> `src/paper/formatting.py` in the codebase graph before Phase 2 begins.
...
> `--no-save` is `action="store_true"` (default `False`) — the script **saves by default**
> and the EOD cron (`python scripts/paper_3track_snapshot.py --date ...`) runs without any flag.
> After renaming to `--dry-run / --no-dry-run` with `default=True`, that same cron call becomes
> a silent no-op. **The cron entry and all doc examples must be updated in this commit** or the
> change should not land. See required doc updates below.
...
#### Required doc updates (must land in this commit — not a follow-up)
- `docs/instructions/3track.md` — update all `--no-save` examples to `--no-dry-run`; update
  cron entry to append `--no-dry-run`.
...
- docs/instructions/3track.md: update all --no-save → --no-dry-run; cron example updated
- docs/instructions/paper_trade.md: same --no-save sweep
...
- Clarify interaction: `--index` and `--overlay` are independent filters; both may be supplied
  simultaneously. Neither bypasses the DTE gate unless `--force` is also passed.

#### [MODIFY] [find_strike_by_delta.py](file:///Users/abhadra/myWork/myCode/python/NiftyShield/scripts/find_strike_by_delta.py) [CLI-5]
- Add `--track spot|futures|proxy` shortcut (maps to the appropriate underlying/instrument).

#### [NEW] tests for Phase 3 changes
- `tests/unit/paper/test_paper_3track_snapshot.py` — add cases for `--verbose` gating (verbose
  output absent when flag is off; present when on).
- `tests/unit/paper/test_paper_3track_overlay_roll.py` — add cases for `--index` and `--overlay`
  filtering logic.
...
- scripts/paper_3track_snapshot.py: summary table before verbose; add --verbose/-v flag
...
- Guard: if running non-interactively (e.g. cron / piped stdin) and `--yes` is absent,
  raise a clear error rather than hanging on `input()`.
...
- `tests/unit/paper/test_paper_3track_overlay_roll.py` — confirmation prompt: invoked without
  `--yes`; skipped with `--yes`; non-interactive guard raises cleanly.
...
- tests/unit/paper/test_paper_3track_overlay_roll.py: confirmation prompt cases
Ref: none
```
...
- Phase 2: existing tests must stay green; no new tests for pure flag renames (tested by `--help` + docstring inspection).
