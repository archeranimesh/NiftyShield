...
Now that you have successfully connected the `codebase-memory-mcp` to Antigravity, I have direct access to your knowledge graph. Here is exactly how to interact with me to maximize efficiency, avoid
context bloat, and utilize your custom agents.
...
* **Rely on the MCP Graph:** Because I have the `codebase-memory-mcp` tools, I can semantically search the codebase. If you ask me to look for the "Nuvama intraday snapshot logic", I don't need you to
  feed me files; I will use `mcp_codebase-memory-mcp_search_graph` to find the exact nodes and read only the necessary code snippets.
* **Artifact Distillation:** When I read large files, I will distill my findings into my own local "Artifacts" (like markdown summaries) so I remember the plan without keeping the entire raw file in
