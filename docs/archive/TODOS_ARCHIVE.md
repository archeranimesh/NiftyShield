...
OpenRouter per-call token usage + USD cost captured on every signal provider response via inline `usage.include` accounting (no `/generation` call), persisted as three nullable `signal_responses` columns (`prompt_tokens` / `completion_tokens` / `cost_usd`), aggregated by `SignalStore.get_signal_cost(from_date, to_date)`, and shown as today's three-call spend on the 09:30 morning Telegram message (all three variants). SHAs: SCT-1 `a519714`, SCT-2 `c7efe54`, SCT-3 `1078397`, SCT-4 (docs) `b70f8fa`. Cost aggregates trustworthy from 2026-09-11. No backfill of pre-SCT-2 rows. Folder: `docs/archive/plan/signals-cost-tracking/`.

### ✅ signals-paper-track — DONE 2026-09-11 (SPT-1..8)

Turned the `signals/` daily multi-LLM consensus into a live paper-traded strategy: `SignalTrackV1` (`paper_signal_track_v1`) on the shared `StrategyMonitor` / `PaperExecutor` / `PaperStore`, auto-entering the daily `DailySignal` as a long monthly option (near-month, rolled to next-month at 14 DTE via `resolve_monthly_option`), managing it intraday against a fixed SL −30 % / target +50 % via the pure `src/strategy/signal_exit.py` evaluator at 30 s monitor cadence, exiting on a hit or 15:00 square-off, and logging the full mark path (`paper_signal_entries` / `paper_signal_marks`) for a 6-month evaluation window feeding SPT-7's G1–G9 go-live gate report. Module boundary ruled by council q17 (2026-09-09, `docs/archive/council/strategy/2026-09-09_signals-paper-track-execution-layer.md`). Entrypoint wiring added no new cron — `morning_signal.py`'s guarded tail-call opens the entry, `scripts/signal_paper_entry.py` is the manual `--date` backfill tool. SPT-5's exit message supersedes `signals/` S5.5a's Phase-1 interim outcome message (marked `won't-do`). SHAs: SPT-1 `636c190`, SPT-2 `58e0b08`, SPT-2a (no-op, docs-only), SPT-3 `6b0dada`, SPT-3b `c050de0`, SPT-4 `7b11e83`, SPT-5 `06df9d8`, SPT-6 `1f52880`, SPT-7 `71da2d5`. Go-live gate + pilot unexercised — 6-month window not yet elapsed. Folder: `docs/archive/plan/signals-paper-track/`.

### ✅ signals-entrypoint-consolidation — DONE 2026-09-12 (SEC-1..6)

Collapsed the duplicated glue across the four signal entrypoints: one holiday guard (`market_calendar.guard_trading_day` + `is_market_session_now`, adopted at every entrypoint and in `StrategyMonitor` — gave `record_signal_outcome` the guard it lacked), one `DailySignal.is_actionable` fire-check predicate replacing four hand-written `trade_action is not NO_TRADE` checks, and `scripts/record_signal_outcome.py` + `scripts/signal_report.py` merged into `scripts/signal_eod.py` (record phase then report phase, one 16:00 cron instead of two, byte-identical `SignalOutcome` baseline row). `morning_signal.run()`'s pipeline body was extracted into `src/signals/pipeline.py::run_morning_signal_pipeline`, leaving `scripts/morning_signal.py` orchestration + Telegram only. `scripts/signal_paper_entry.py` keep-or-delete was reviewed and deferred — the SPT-6 tail-call had zero live production runs yet, so neither criterion was met; kept as the documented backfill tool. SEC-6 also caught and fixed a live gap: the Mac host crontab still ran the two scripts SEC-3 retired, which no longer existed — Animesh applied the corrected single 16:00 `scripts.signal_eod` line, verified via `crontab -l`. SHAs: SEC-1 `1ef2974`, SEC-2 `0202291`/`819306b`, SEC-3 `928cb22`/`b297734`/`6b6177b`, SEC-4 `85b4744`, SEC-5 `290ba3a`. Folder: `docs/archive/plan/signals-entrypoint-consolidation/`.
