Read `CONTEXT.md` and state `CONTEXT.md ✓` before doing anything else. Then read `docs/plan/paper-backbone-adj/paper_backbone_adj_tasks.md` and find the first unchecked box — the first `- [ ]` line. That is your **only task** for this session. Do not look at any other unchecked item. Do not attempt to batch or combine tasks. One task. Complete it fully. Move on to nothing else.

**Story spec:** Read `docs/plan/paper-backbone-adj/stories/<TASK_ID>.md` for the full implementation spec, pre-baked graph context, test list, and commit message. Follow it exactly.
...
**Verify and record:** Copy the SHA from `git log --oneline -1`. Open `docs/plan/paper-backbone-adj/paper_backbone_adj_tasks.md`, change `- [ ]` to `- [x]` on the completed line, and append `| SHA: <sha>`. Then add one line to `TODOS.md` under the session log:
`| <YYYY-MM-DD> | paper-backbone-adj <task-id> — <one-line description> — <SHA> |`
