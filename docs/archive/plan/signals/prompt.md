Read `CONTEXT.md` and state `CONTEXT.md ✓` before doing anything else. Then read `docs/plan/signals/signals_tasks.md` and find the first unchecked box
...
**Story spec:** Read the matching story in `docs/plan/signals/signals_stories.md` (same task ID) for the full implementation spec,
...
**Verify and record:** Copy the SHA from `git log --oneline -1`. Open `docs/plan/signals/signals_tasks.md`, change `- [ ]` to `- [x]` on the completed line,
