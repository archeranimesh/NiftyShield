...
**Story complete — SWEEP-1..SWEEP-7 all landed.** Epic `token-efficiency/` is ready to
archive (all three stories done); see `docs/plan/README.md` §Conventions *Completion → archive*.
...
- [x] **SWEEP-1** — Cluster every `suggestions.md` row into ~7 groups; write the cluster map + each row's outcome (fix / enforce / accept + reason) into the SWEEP-1 As-built |
      Owner: Claude | Model: claude-sonnet-5 | Review: none | SHA: 622465c
...
- [x] **SWEEP-4** — Commit-flow cluster: `scripts/dev/commit_preflight.py` (staged-index, `ruff format --check`, md-line-length, next-marker, `<pending>`-SHA policy) wired into the `commit` skill |
      Owner: Claude | Model: claude-sonnet-5 | Review: code-reviewer | SHA: 2b85b84
- [x] **SWEEP-5** — MCP / tool-param cluster: document the `codebase-memory-mcp` required-params, the `AskUserQuestion` plain-array rule, and the "don't `ScheduleWakeup`-poll a spawned agent" rule |
      Owner: Claude | Model: claude-sonnet-5 | Review: none | SHA: 2ae951c
