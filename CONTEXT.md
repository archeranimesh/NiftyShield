...
`src/paper/` — paper trading module. `TradeState` enum (`OPEN`/`DEFENDED`/`RE_ENTRY_PENDING`) in `models.py`. `PaperTrade` model (frozen Pydantic, `paper_` prefix enforced, includes `ivr_at_entry: float | None`, `state: TradeState = OPEN`), `PaperPosition` + `PaperNavSnapshot` + `PaperLegSnapshot` (frozen dataclasses), `PaperStore` (`paper_trades` + `paper_nav_snapshots` + `paper_leg_snapshots` + `pending_approvals` + `council_outputs` + `daemon_heartbeat` + `paper_exit_events` tables in shared SQLite), `PaperTracker` (compute_pnl + record_daily_snapshot). `PaperStore` API: `record_leg_snapshot` (upsert; enforces `total_pnl == unrealized_pnl + realized_pnl`), `get_leg_snapshot`, `get_prev_leg_snapshot`, `delete_trade` (no-op if missing; WHERE includes `instrument_key` to prevent wrong-row deletion during rolls), `delete_trade_by_id(trade_id)` (preferred rollback primitive when PaperTrade object unavailable), `create_exit_event` (upsert by trade_id + exit_signal), `get_open_exit_events` (filter by strategy/leg/status), `update_trade_state(trade_id, TradeState)` (raises ValueError if id missing). `paper_exit_events` schema captures: strategy_name, leg_name, trade_id, exit_signal (`ExitSignal` enum), severity (INFO/WARN/ACTION), ltp/mid/bid/ask/delta/dte, entry_price, threshold_value, dual-signal audit fields (`delta_stop_would_fire`, `premium_stop_would_fire`, `actual_rule_used`), status (OPEN/ACKNOWLEDGED/ACTED/DISMISSED). `PaperExitEvent` frozen Pydantic model in `src/paper/models.py`. See `src/paper/CLAUDE.md` for full invariants.
`src/strategy/` — paper-backbone strategy layer. `PaperStrategy` protocol (`protocol.py`) + `SignalEvent` + `ApprovedAction` + `LegSpec` models. `ReEntryMixin` (`reentry_mixin.py`) providing R5 re-entry checks; inherited by `CSPNiftyV1` (CC-3). `StrategyMonitor` (`monitor.py`): registry + tick loop + signal routing + heartbeat. `PaperExecutor` (`executor.py`): action dispatch + `PaperFillSimulator` (VIX-regime slippage model). Concrete strategies: `CSPNiftyV1` (`csp_nifty_v1.py`), `IronCondorV1` (`ic_nifty_v1.py`), `NiftyTrackComparisonV1` (`nifty_track_comparison_v1.py`). All implement `PaperStrategy` protocol. `ExitSignalEngine` (`exit_signals.py`): static rule engine for CSP, CC, PP, and Collar overlay rule sets; returns `ExitSignalResult` with severity (INFO/WARN/ACTION). CSP uses 5 independent classmethods (CR1b): `evaluate_profit_target_csp` (LTP ≤ 30% of entry credit = 70% captured), `evaluate_hard_stop_csp` (LTP ≥ 2× entry), `evaluate_delta_breach_csp` (|δ| ≥ 0.40; state-aware: OPEN→DELTA_BREACH, DEFENDED→DELTA_BREACH_FINAL), `evaluate_time_stop_csp` (days_held ≥ 21), `evaluate_roll_eligible_csp` (DTE ≤ 7). `CSPNiftyV1.apply_action` accepts `CLOSE_FULL`, `PROFIT_TARGET`, and `TIME_STOP`; both `PROFIT_TARGET` and `TIME_STOP` trigger `ReEntryMixin._check_reentry`. Module constant `_PROFIT_TARGET_RETENTION = Decimal("0.30")`. CC/PP/Collar use combined evaluators with dual-signal audit fields; thresholds: CC loss stop 2.5×, delta stop +0.55; PP CRASH_MONETIZE when δ ≤ −0.80 or value ≥ 5× debit; Collar 75% decay on short call. Overlay strategies: `CCOverlayV1` (`cc_overlay_v1.py`), `PPOverlayV1` (`pp_overlay_v1.py`), `CollarOverlayV1` (`collar_overlay_v1.py`) — all implement `PaperStrategy`; emit exit/warning signals for standalone CC, PP, and Collar option legs respectively. All ACTION-severity `SignalEvent` payloads across all strategies include `"valid_actions": [...]` (the list of `ApprovedAction.action_type` strings accepted by that strategy's `apply_action`), used by `TelegramGateway.send_approval_request` to build the approval keyboard without a council call. `OverlayCloser` (`overlay_closer.py`): atomic multi-leg close orchestrator; `close_collar_all` and `monetize_collar_put` use `store.record_trades([...])` to commit both legs in a single DB transaction — no application-level rollback needed. `monetize_collar_put` validates put leg exists before touching call (incomplete-collar guard). CC single-leg and PP single-leg use `record_trade` (single-leg, always atomic). `CSPNiftyV1` extended with R5 re-entry eligibility check (`_check_r5_reentry`) on `PROFIT_TARGET` action: gates on DTE ≥ 14, IVR threshold, no open position already exists; writes `R5_REENTRY_ELIGIBLE` or `R5_REENTRY_BLOCKED` event to `paper_exit_events`. `auto_close.py` (AUTO-1) encapsulates EOD auto-close execution and routing via `OverlayCloser` for all overlays, status resolution as `ACTED`, unified Telegram close notification dispatch, and EOD PP re-entry eligibility checks (`evaluate_pp_reentry_eod`).
