...
`pyproject.toml` — project metadata, all dev dependencies (`pytest`, `ruff`, `mypy`, `pre-commit`, `vulture`, `bandit`, `commitizen`), `[tool.pytest.ini_options]` (testpaths, asyncio_mode, markers), `[tool.ruff]` lint/format rules, `[tool.mypy]` with phased strict rollout (strict on `src/client.*` + `src/paper.*`; permissive elsewhere; third-party stubs silenced). Install: `pip install -e ".[dev]"`.
`Makefile` — targets: `test`, `coverage`, `lint`, `fmt`, `security`, `dupes`, `dead-code`, `ci`, `index`. `make ci` = lint → test → coverage → security. Single entry point for all dev and CI checks.
