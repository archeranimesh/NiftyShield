...
- **2026-09-09** — `signals/` LLM cost capture: chose OpenRouter **inline usage accounting** (`"usage": {"include": true}` in the request body → `usage.cost` + native token counts on the same
  response envelope) over the `GET /generation?id=` lookup (one extra HTTP call per provider, response lag, needs a retry) and over `/credits` + `/activity` (key-lifetime totals, no per-run grain).
  Zero added calls, exact per-call cost, pipeline already parses the envelope; aggregation is a local SQLite `SUM` over three new `signal_responses` columns. Requested by Animesh.
  _(source: `docs/plan/signals-cost-tracking/prompt.md`)_
