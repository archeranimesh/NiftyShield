...
def _find_strike_by_delta(
    chain: OptionChain, side: Literal["CE", "PE"], target_delta: Decimal
) -> Decimal | None:
...
async def select_overlay_expiry(
    ...
    for expiry in candidate_expiries:
        try:
            ...
            raw_chain = await asyncio.wait_for(
                broker.get_option_chain(underlying_key, expiry), timeout=timeout_sec
            )
        ...
        if option_type == "COLLAR":
            ...
            if target_strike is not None and target_strike in chain.strikes:
                leg = (
                    chain.strikes[target_strike].ce
                    if option_type == "CE"
                    else chain.strikes[target_strike].pe
                )
                if leg is not None:
                    mid = (
    ...
    for i, profile in enumerate(profiles):
        if isinstance(profile, CollarSpreadProfile):
            if profile.put_spread_pct is not None and profile.call_spread_pct is not None:
                ...
                if max_spread <= GATE:
                    reason = (
                        f"Gate passed ({max_spread:.2f}% <= 3%) at preference rank {i + 1}"
                        if i > 0
                        else None
                    )
            ...
            if profile.spread_pct is not None:
                if profile.spread_pct <= GATE:
                    reason = (
                        f"Gate passed ({profile.spread_pct:.2f}% <= 3%) at preference rank {i + 1}"
                        if i > 0
                        else None
                    )
                    return OverlaySelection(profile.expiry, profiles, reason)
    ...
    return OverlaySelection(
        fallback_expiry, profiles, "All candidate expiries failed the 3% spread gate."
    )
